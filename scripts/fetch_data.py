        ...
        for s in list(sponsors) + list(cosponsors):
            ...
            if bid:
                ...
                if cls == "anti_arms" and bid in members_by_id:
                    ...
                    members_by_id[bid]["antiArmsLevel"] = "hard"   # legislative action = hard
...
def enrich_hard_classification(members, bills):
    """
    Uses Congress.gov /member/{bioguideId}/cosponsored-legislation to
    directly verify whether each anti-arms member has co-sponsored any
    tracked anti-arms bills. Upgrades soft → hard where confirmed.
    Also checks /house-vote for No votes on pro-arms bills (119th Congress).
    Non-critical — if Congress.gov is unavailable, TrackAIPAC soft labels remain.
    """
    if not CONGRESS_KEY:
        return members

    anti_bill_ids = {b["id"] for b in bills if b.get("classification") == "anti_arms"}
    pro_bill_ids  = {b["id"] for b in bills if b.get("classification") == "pro_arms"}
    anti_bill_nums= {b["number"].lower() for b in bills if b.get("classification") == "anti_arms"}

    # Map bill number → uid for matching
    num_to_uid = {b["number"].lower(): b["id"] for b in bills}

    soft_members = [m for m in members if m.get("antiArms") and m.get("antiArmsLevel") == "soft"
                    and m.get("bioguide_id")]

    if not soft_members:
        return members

    print(f"\n  Congress.gov: checking {len(soft_members)} soft anti-arms members for hard evidence…")
    upgraded = 0

    for m in soft_members:
        bid = m["bioguide_id"]
        # Fetch their co-sponsored legislation
        url = congress_url(f"/member/{bid}/cosponsored-legislation",
        ...
        for item in d.get("cosponsoredLegislation", []):
            ...
            if uid in anti_bill_ids or bnum.lower() in anti_bill_nums:
                ...
                if uid not in m.get("bills", []):
                    m.setdefault("bills", []).append(uid)
                upgraded += 1
...
def main():
    ...
    # ── Congress.gov: upgrade soft → hard where co-sponsorship confirmed ────────
    members = enrich_hard_classification(members, bills)
    members_by_name = {m["name"].lower(): m for m in members}  # re-index
    ...
    data = {
        "meta": {
            ...
            "hard_count":         sum(1 for m in members if m.get("antiArmsLevel")=="hard"),
            "soft_count":         sum(1 for m in members if m.get("antiArmsLevel")=="soft"),
